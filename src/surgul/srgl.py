"""
SRGL: Formally Verified Safety-First Logic for Clinical Triage
...
class SRGL:
    """
    SRGL: Formally Verified Safety-First Clinical Triage System
...
def quick_predict(patient_data: Dict, verbose: bool = False) -> str:
    ...
    """
    srgl = SRGL()
    decision = srgl.predict(patient_data)

    if verbose:
        return srgl.explain(patient_data, verbose=True)
    else:
        return f"Risk: {decision.risk_tier} | Action: {decision.action} | Time: {decision.decision_time_ms:.1f}ms"


if __name__ == '__main__':
    print("SRGL System Demo\n")
    ...
    srgl = SRGL()
    print(srgl.explain(stroke_patient, verbose=False))
    ...
    print(f"\nFull explanation:\n{srgl.explain(bppv_patient, verbose=True)}")
